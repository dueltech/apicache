...
const apis = [
  ...
  {
    name: "express+gzip",
    server: require("./api/express-gzip"),
  },
];
...
describe(".options(opt?) {GETTER/SETTER}", function () {
  const apicache = require("../src/apicache");
...
describe(".getDuration(stringOrNumber) {GETTER}", function () {
  ...
  describe("unit support", function () {
    ...
    it("weeks", function () {
      expect(apicache.getDuration("5 weeks")).to.equal(
        1000 * 60 * 60 * 24 * 7 * 5
      );
    });
    it("months", function () {
      expect(apicache.getDuration("6 months")).to.equal(
        1000 * 60 * 60 * 24 * 30 * 6
      );
    });
  });
...
describe(".getPerformance()", function () {
  ...
  it("returns a null hit rate if the api has not been called", function () {
    ...
    const app = api.create("10 seconds", {
...
describe(".middleware {MIDDLEWARE}", function () {
  ...
  it("returns the middleware function", function () {
    const middleware = require("../src/apicache").middleware("10 seconds");
    expect(typeof middleware).to.equal("function");
    expect(middleware.length).to.equal(3);
  });

  describe("options", function () {
    const apicache = require("../src/apicache").newInstance();
    ...
    describe(api.name + " tests", function () {
      ...
      it("does not cache header in headerBlacklist", function () {
        ...
        return request(app)
          ...
          .expect(200, movies)
          .then(function (res) {
            expect(res.headers["x-blacklisted"]).to.equal(
              res.headers["x-notblacklisted"]
            );
            return request(app)
              .get("/api/testheaderblacklist")
              .set("Accept", "application/json")
      ...
      it('allows defaultDuration to be a parseable string (e.g. "1 week")', function (done) {
        ...
        const app = mockAPI.create(null, {
          defaultDuration: "10ms",
          events: {
            expire: cb,
          },
        });
      ...
      it("can clear indexed cache groups", function () {
        ...
        return request(app)
          ...
          .then(function (res) {
            expect(app.requestsProcessed).to.equal(1);
            expect(app.apicache.getIndex().all.length).to.equal(1);
            expect(app.apicache.getIndex().groups.cachegroup.length).to.equal(
              1
            );
            expect(
              Object.keys(app.apicache.clear("cachegroup").groups).length
            ).to.equal(0);
            expect(app.apicache.getIndex().all.length).to.equal(0);
      ...
      it("clears empty group after removing last specific endpoint", function () {
        ...
        return request(app)
          ...
          .then(function (res) {
            ...
            expect(
              Object.keys(app.apicache.clear("/api/testcachegroup").groups)
                .length
            ).to.equal(0);
            expect(app.apicache.getIndex().all.length).to.equal(0);
          });
